...
"jaoiiae (81)",
"pucsbv (39)"}; // input, reponse : mwzaxaj
  /*vector<string> tab = {"pbga (66)",
                        "xhth (57)",
                        "ebii (61)",
                        "havc (66)",
                        "ktlj (57)",
...
const string explode(const string& s, const char& c) { 
  string buff = "";
  
  for(auto n:s) { // on parcours toute la chaine de caractère 
    if(n != c) { buff+=n; } else // si caractere diff du separateur on fait le XOR
    if(n == c && buff != "") { return buff; } // si on trouve le caractère et que la chaine n'est pas vide, on l'ajoute au vecteur
  } 
