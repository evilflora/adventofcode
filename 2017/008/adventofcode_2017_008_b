...
const string explode2(const string&, const char&);
void calcul(short, vector<string>&, map<string, map<int, int>>&);

int main() {
  clock_t begin = clock();
  
  vector<string> tab = {"b inc 580 if bkd > -1",
"v inc 491 if z >= -1",
...
"fw dec -971 if fz < 1922"}; // input, reponse 6619
  /*vector<string> tab = {"b inc 5 if a > 1",
                        "a inc 1 if b < 5",
                        "c dec -10 if a >= 1",
                        "c inc -20 if c == 10"};*/ // input test, réponse 10
  map<string, int> mymap; // par exemple mymap["a"] = 5;
  vector<string> current;
  short len = tab.size();
  int max;
  
  for(int i = 0; i < len; i++) { // init des différentes variables
    mymap[explode2(tab[i], ' ').data()] = 0; // à 0
  }
  
  for(int i = 0; i < len; i++) { // on analyse les lignes
    current = explode(tab[i], ' '); // ligne actuelle
    short b = stoi(current[2]); 
    short value = (current[1] == "inc" ? 1 : -1) * b;
    
    if (current[5] == "<") {
      if (mymap[current[4]] <  stoi(current[6])) mymap[current[0]] += value; }
    else if (current[5] == "<=") {
      if (mymap[current[4]] <= stoi(current[6])) mymap[current[0]] += value; }
    else if (current[5] == ">") {
      if (mymap[current[4]] >  stoi(current[6])) mymap[current[0]] += value; }
    ...
    if (mymap[current[0]] > max) max = mymap[current[0]];
  }
...
const string explode2(const string& s, const char& c) { 
  string buff = "";
