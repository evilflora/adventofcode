...
int main() {
  ...
  int x = 0, y = 0, i = 0; // point de depart
  short len = tab.size(); // le nombres de commandes à executer
  
  for(int i = 0; i < len; i++) { // transformer en while
   short len = 0;
   direction = (direction + (tab[i][0] == 'R' ? 1 : 3) ) % 4;
   switch(direction) {
    case 0: // Nord
      len = y + get_weight(tab[i]);
      for(; y < len; y++) if(++memory[x][y] > 1) break;
    break;
    case 1: // Est
      len = x + get_weight(tab[i]);
      for(; x < len; x++) if(++memory[x][y] > 1) break;
    break;
    case 2: // Sud
      len = y - get_weight(tab[i]);
      for(; y > len; y--) if(++memory[x][y] > 1) break;
