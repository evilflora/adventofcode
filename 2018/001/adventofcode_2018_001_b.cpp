...
int main() {
  ...
  while (1) {
    result += input[index]; // on additionne la case de l'input
    if ((it = tab.find(result)) != tab.end()) { break; } // on regarde s'il est dans notre liste des valeurs connues
    tab[result]; // on l'ajoute à notre liste
    ++index%=max;  // on va à l'index suivant, si on est au bout du tableau on retour au début
  };
