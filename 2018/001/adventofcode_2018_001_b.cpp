...
int main() {
	...
	while (!stop) {
		...
		if (find(tab.begin(), tab.end(), result) != tab.end()) { printf("aaaaaaa\r\n"); stop = true; break; } // on regarde s'il est dans notre liste des valeurs connues
		tab.push_back(result); // on l'ajoute à notre liste
		if (++index == max) { index = 0; boucle++;  } // on va à l'index suivant, si on est au bout du tableau on retour au début
	};
